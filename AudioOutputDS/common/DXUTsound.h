...
public:
    ...
    HRESULT Play(DWORD playPos,  DWORD dwPriority = 0, DWORD dwFlags = 0, LONG lVolume = 0, LONG lFrequency = -1, LONG lPan = 0 );
    HRESULT Play3D( LPDS3DBUFFER p3DBuffer, DWORD dwPriority = 0, DWORD dwFlags = 0, LONG lFrequency = 0 );
    HRESULT Stop();
    HRESULT Reset();
    BOOL    IsSoundPlaying();
	HRESULT GetPlayPos(DWORD & playPos);
	HRESULT SetPlayPos(DWORD playPos);
};
