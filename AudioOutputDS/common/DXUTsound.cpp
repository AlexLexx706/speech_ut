...
//-----------------------------------------------------------------------------
HRESULT CSound::Play(DWORD playPos, DWORD dwPriority, DWORD dwFlags, LONG lVolume, LONG lFrequency, LONG lPan )
{
    HRESULT hr;
    BOOL    bRestored;
    ...
    if( FAILED( pDSB->SetCurrentPosition(playPos) ) )
        return DXUT_ERR( L"SetCurrentPosition", hr );
...
HRESULT CSound::GetPlayPos(DWORD & playPos)
{
	if( m_apDSBuffer == NULL )
		return CO_E_NOTINITIALIZED;
	...
	DWORD writePos;
	for( DWORD i=0; i<m_dwNumBuffers; i++ )
		hr = m_apDSBuffer[i]->GetCurrentPosition(&playPos, &writePos);
    return hr;
}

HRESULT CSound::SetPlayPos(DWORD playPos)
{
	if( m_apDSBuffer == NULL )
		return CO_E_NOTINITIALIZED;
	...
	DWORD writePos;
	for( DWORD i=0; i<m_dwNumBuffers; i++ )
		hr = m_apDSBuffer[i]->SetCurrentPosition (playPos);
    return hr;
}
